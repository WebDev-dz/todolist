...
// Example prompts updated to generate multiple tasks
...
const GenerateTaskWithAIPage: React.FC = () => {
  ...
  const { addTask, tasks: previousTasks } = useTaskStore();
  ...
  const generateTaskWithAI = async (userPrompt: string, uri?: string | null ) => {
    setIsGenerating(true);
    
    try {
      // Modify the prompt to specifically request JSON format
      console.log({prompt, selectedImage})


      const response = await useFetchAi(userPrompt,previousTasks, uri);
...
const takePhoto = async () => {
  ...
  const handleSaveTasks = () => {
    ...
    selectedTasks.forEach((st) => addTask(st))
    router.back();
  };

  const handlePromptSubmit = () => {
    // if (prompt.trim()) {
      generateTaskWithAI(prompt.trim(), selectedImage);
    // }
  };
              ...
              {recentPrompts.map((recentPrompt, index) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    generateTaskWithAI(recentPrompt);
                  }}
                  className="flex-row items-center p-3 bg-gray-50 rounded-xl"
                >
